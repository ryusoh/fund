...
def load_holdings(filepath: Path) -> HoldingsType:
    ...
    try:
        ...
        if isinstance(loaded_data, list):
            ...
            logging.warning(f"Holdings file {filepath} contains data in an unexpected format. Starting with empty holdings.")
            return {}
    except (json.JSONDecodeError, IOError) as e:
        logging.error(f"Error loading holdings file {filepath}: {e}. Starting with empty holdings.")
        return {}
    except InvalidOperation as e:
        logging.error(f"Error converting value to Decimal in {filepath}: {e}. Please check file content. Starting with empty holdings.")
        return {}
...
def buy_shares(holdings: HoldingsType, ticker: str, shares_bought_str: str, purchase_price_str: str) -> None:
    ...
    if ticker in holdings:
        ...
        logging.info(f"New holding for {ticker}: {new_total_shares} shares at average price ${new_average_price:.2f}.")
    else:
        holdings[ticker] = {
            'shares': shares_bought,
            'average_price': purchase_price
        }
        logging.info(f"Added new ticker {ticker}: Bought {shares_bought} shares at ${purchase_price:.2f}.")

def sell_shares(holdings: HoldingsType, ticker: str, shares_sold_str: str, sell_price_str: str) -> None:
    ...
    if shares_sold <= 0 or sell_price < 0:
        logging.error("Number of shares to sell must be positive, and sell price must be non-negative.")
        return
    ...
    if shares_sold > current_shares:
        logging.error(f"Cannot sell {shares_sold} shares of {ticker}. You only own {current_shares} shares.")
        return
    ...
    if remaining_shares == Decimal('0'):
        ...
        logging.info(f"Remaining holding for {ticker}: {remaining_shares} shares at average price ${current_avg_price:.2f}.")
...
def list_holdings(holdings: HoldingsType) -> None:
    ...
    for ticker, data in sorted(holdings.items()):
        ...
        logging.info(f"{ticker:<10} {str(shares):>15} {f'${avg_price:.2f}':>15} {f'${cost_basis:.2f}':>15}")
    logging.info(separator)
...
def main() -> None:
    ...
    parser.add_argument('--file', type=Path, default=DEFAULT_HOLDINGS_FILE_PATH,
