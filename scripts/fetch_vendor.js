...
(async () => {
  try {
    ...
    for (const asset of assets) {
      ...
      manifest.push({ url: asset.url, file: path.relative(path.join(__dirname, '..'), asset.out), sha256 });
      // eslint-disable-next-line no-console
      console.log(`Fetched: ${asset.url} -> ${asset.out}`);
    }
    const manifestDir = path.join(__dirname, '..', 'assets', 'vendor');
    fs.mkdirSync(manifestDir, { recursive: true });
    fs.writeFileSync(path.join(manifestDir, 'manifest.json'), JSON.stringify({ generatedAt: new Date().toISOString(), files: manifest }, null, 2));
  } catch (e) {
    // eslint-disable-next-line no-console
