...
describe('initCurrencyToggle – inner element click path', () => {
    ...
    `;
        jest.resetModules();
        const {
            initCurrencyToggle: initCurrencyToggleLocal,
        } = require('@ui/currencyToggleManager.js');
        document.getElementById('currencyToggleContainer');
        // Buttons queried via closest() path; no need to store here
        jest.clearAllMocks();
        jest.spyOn(document, 'dispatchEvent');
...
describe('initCurrencyToggle – branch 25 coverage (same currency first, then change)', () => {
    ...
    `;
        ...
        document.getElementById('currencyToggleContainer');
        // Buttons are interacted via inner spans; no need to store here
        jest.clearAllMocks();
        jest.spyOn(document, 'dispatchEvent');
        initCurrencyToggleLocal();
    });
