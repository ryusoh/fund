...
## 2026-05-12 - Custom Dropdown Keyboard Accessibility

**Learning:** When creating custom dropdown menus (e.g., table filter dropdowns) using `div` elements generated by JavaScript, these elements are inherently inaccessible to keyboard users. They do not receive focus and cannot be activated via the keyboard, severely limiting accessibility for power users and assistive technologies.

**Action:** Always add `role="button"`, `tabindex="0"`, and a `keydown` event listener (handling `Enter` or `Space`) to interactive custom `div` elements functioning as options or buttons. Additionally, provide an explicit `:focus-visible` styling (e.g., `outline: 2px solid rgba(255, 255, 255, 0.5)`) to ensure the focus state is clearly visible.
