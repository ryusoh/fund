...
| Date       | Incident / Vulnerability                              | Category              |
| :--------- | :---------------------------------------------------- | :-------------------- |
| 2026-03-11 | [DOM XSS via Template Literals in Terminal Crosshair] | DOM XSS               |
| 2026-03-10 | DOM XSS via Error Messages & Dynamic Data Rendering   | DOM XSS               |
| 2026-03-09 | DOM XSS via External Markdown Headers                 | DOM XSS               |
| 2026-03-09 | [DOM XSS in Transaction Data Table Rendering]         | DOM XSS               |
| 2024-05-24 | Cross-Site Scripting (XSS) via Error Messages         | DOM XSS               |
| 2024-05-24 | [Terminal DOM XSS Fix]                                | DOM XSS               |
| 2024-03-08 | [Insecure API Keys Transmission]                      | Insecure Transmission |

---

## 2026-03-11 - [DOM XSS via Template Literals in Terminal Crosshair]

**Vulnerability:** A High Priority DOM XSS vulnerability existed in `js/transactions/terminal.js` where the `updateTerminalCrosshair` function used `innerHTML` to inject dynamically constructed HTML elements populated from potentially untrusted data (`snapshot.series` and `rangeSummary` entries such as `label`, `formatted`, etc.).

**Learning:** This occurred because template strings offer a very concise and convenient way to assemble complex, multiline UI elements in JavaScript, whereas building them with safe DOM manipulations (like `document.createElement`) requires significantly more lines of code. This convenience led to the bypassing of safe text rendering.

**Prevention:** To avoid this next time, always construct DOM nodes manually or use a framework/templating engine that auto-escapes by default. When handling dynamically formatted text, create a generic builder utility function that simplifies applying `textContent` to nested elements, reducing the verbosity of safe DOM methods and making the safe path the easier path.
...
## 2026-03-09 - [DOM XSS in Transaction Data Table Rendering]
...
## 2024-05-24 - [Terminal DOM XSS Fix]

**Vulnerability:** Direct concatenation of user input into `innerHTML` allows arbitrary JavaScript execution within the client's session.

**Learning:** Data must be treated as untrusted even in terminal-like interfaces.

**Prevention:** Always use `document.createElement()` and assign user-controlled data via `textContent` or `document.createTextNode()` rather than `innerHTML` to ensure the browser treats it as literal text.

## 2024-03-08 - [Insecure API Keys Transmission]

**Vulnerability:** Transmitting secret API keys via HTTP allows MITM attacks.
