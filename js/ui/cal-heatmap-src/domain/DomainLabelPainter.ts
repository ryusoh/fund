...
export default class DomainLabelPainter {
    ...
    paint(root: any): void {
        ...
        root.selectAll(DEFAULT_SELECTOR)
            .data(
                (d: Timestamp) => [d],
                (d: Timestamp) => d
            )
    ...
    #getDomainInsideHeight(d: Timestamp): number {
        ...
        return (
            coordinates.get(d)!.inner_height - domainVerticalLabelHeight - verticalPadding(padding)
        );
    }
        ...
        switch (rotate) {
            ...
            case 'right':
                selection.attr('transform', (d: Timestamp) => {
                    ...
                    const s = [`rotate(90, ${position === 'right' ? domainWidth : labelWidth}, 0)`];
