...
export default class DomainCoordinates {
    ...
    #getWidth(d: Timestamp): number {
        ...
        const columnsCount = this.calendar.templateCollection.get(subDomain.type)!.columnsCount(d);
    ...
    #getHeight(d: Timestamp): number {
        const { domain, subDomain, x, verticalOrientation } = this.calendar.options.options;
        const rowsCount = this.calendar.templateCollection.get(subDomain.type)!.rowsCount(d);
