...
export async function handlePercentageCommand(args, context) {
    const { appendMessage, chartManager } = context;
    let result = '';
    const chartSection = document.getElementById('runningAmountSection');
    const isChartVisible = chartSection && !chartSection.classList.contains('is-hidden');
    ...
    if (!matchedConfig) {
        ...
        const targetChart = matchedConfig.base;

        if (!isChartVisible) {
            result = `${matchedConfig.name} chart must be active. Use \`plot ${matchedConfig.base}\` first.`;
        } else if (activeChart === targetChart) {
            result = `${matchedConfig.name} chart is already showing percentages.`;
        } else {
            updateChartStateAndRender(targetChart, chartManager);
            result = `Switched ${matchedConfig.name.toLowerCase()} chart to percentage view.`;

            const summary = await getChartSnapshot(matchedConfig, false);
            if (summary) {
                result += `
${summary}`;
            }
