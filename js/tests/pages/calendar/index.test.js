...
jest.mock('https://cdn.jsdelivr.net/npm/cal-heatmap@4.2.4/+esm', () => jest.fn().mockImplementation(() => mockCalHeatmapInstance));
...
jest.mock('https://cdn.jsdelivr.net/npm/d3@7/+esm', () => {
    ...
    d3.each.mockImplementation(function(callback) {
        const scenarios = [
            { element: { parentNode: {} }, datum: { t: new Date('2025-01-01T00:00:00Z').getTime() } },
            { element: { parentNode: {} }, datum: { t: new Date('2025-01-02T00:00:00Z').getTime() } },
...
describe('calendar page', () => {
  ...
  beforeEach(() => {
    ...
    document.querySelector = jest.fn().mockImplementation((sel) => {
      if (sel === '#cal-prev') return prevBtnRef;
      if (sel === '#cal-next') return nextBtnRef;
      if (sel === '#cal-today') return todayBtnRef;
      if (sel === '#calendar-container') return containerRef;
  ...
  it('should initialize the calendar and set up event listeners', async () => {
    ...
    const tooltipText = paintArg2.tooltip.text(new Date(), 0.0123, { format: () => '2025-01-01' });
  ...
  it('should handle currency change event and re-render labels', async () => {
    ...
    getCalendarData.mockResolvedValue(mockData);
    await initCalendar();
    document.dispatchEvent(new CustomEvent('currencyChangedGlobal', { detail: { currency: 'JPY' } }));
  ...
  it('should handle mobile start-date branch and tooltip variants', async () => {
    ...
    const mockData = {
      processedData: [
        { date: '2025-02-01', value: 0.0, total: 0 },
      ],
