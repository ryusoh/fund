...
5. **Explicit Agent State & Memory (State-Oriented Architecture, SDN Control Plane & Dynamic Routing)**: The repo should reserve directories for agents to store their execution states, tasks, and schemas (`.agents/state/`). Multi-step agent workflows should never rely on conversation token memory across transaction boundaries ($P = p^N$ degradation); progress should be persisted to disk-backed state ledgers (Arista EOS SysDB & Google Orion model), and large workflows should be partitioned into isolated, stateless gated subagent micro-contexts ($O(1)$ context) with dynamically routed toolsets (Google Jupiter OCS model) to eliminate autoregressive attention dilution.
