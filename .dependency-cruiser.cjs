...
module.exports = {
    forbidden: [
        {
            name: 'no-circular',
            comment:
                'Circular deps make modules untestable in isolation (docs/agentic-quality-gates.md §3)',
            severity: 'error',
            from: {},
            to: { circular: true },
        },
        {
            ...
            comment:
                'AGENTS.md non-negotiable #6: page-scoped changes must not leak to other pages',
            severity: 'error',
            from: { path: '^js/pages/([^/]+)/' },
    ...
    options: {
        ...
        // repo has .ts sources (cal-heatmap-src); they are excluded above and
        // covered by verify-calendar-build.
    },
};
